...
export default function JobApplicationPage({ params }: JobApplicationPageProps) {
  ...
  const [jobDetails, setJobDetails] = useState<JobApplicationResponse | null>(null)
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      ...
      try {
        ...
        setJobDetails({
          ...
          selected_candidates_count: job?.applicant_status_counts?.selected || 0,
        })

        if (candidates.length > 0 && !selectedCandidateId && !isMobile) {
    ...
    fetchJobData()
  }, [jobTitleSlug, experienceFilter, selectedCandidateId, isMobile])
  ...
  const handleStatusUpdate = (status: string) => {
    console.log(`Status updated to: ${status} for candidate: ${selectedCandidateId}`)
  }
  ...
  if (!jobDetails || candidates.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <p className="text-lg text-gray-700">No candidates found for &rdquo;{jobTitleForDisplay}&rdquo;.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-50 max-w-7xl mx-auto">
      <div className="w-full p-4 md:p-6 lg:p-8">
        <div className="mb-4 md:mb-6 lg:mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2 text-sm text-black max-md:mb-5">
              <Image src="/assets/Vector.png" alt="Dashboard icon" width={20} height={20} className="md:w-6 md:h-6 lg:w-7 lg:h-7" />
              <span className="text-xs md:text-xl lg:text-2xl font-bold font-raleway text-[#0A1754]">Dashboard</span>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 md:gap-6 lg:gap-10 mb-0 max-w-4xl md:ml-10 lg:ml-12 font-raleway font-bold">
          <div className="bg-[#0A1754] text-white p-3 md:p-5 lg:p-6 rounded-lg text-center">
            <h3 className="text-[10px] md:text-base lg:text-xl font-medium mb-1">Total Applications</h3>
            <p className="text-xs md:text-2xl lg:text-4xl font-bold">{jobDetails.total_applications}</p>
          </div>
          <div className="bg-[#0A1754] text-white p-3 md:p-5 lg:p-6 rounded-lg text-center">
            <h3 className="text-[10px] md:text-base lg:text-xl font-medium mb-1">Shortlisted Talents</h3>
            <p className="text-xs md:text-2xl lg:text-4xl font-bold">{jobDetails.shortlisted_talents}</p>
          </div>
          <div className="bg-[#0A1754] text-white p-3 md:p-5 lg:p-6 rounded-lg text-center">
            <h3 className="text-[10px] md:text-base lg:text-xl font-medium mb-1">Selected Candidate</h3>
            <p className="text-xs md:text-2xl lg:text-4xl font-bold">{jobDetails.selected_candidates_count}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row flex-1 h-full">
        <div
          className={`flex-1 w-full md:max-w-[50%] px-4 md:px-6 lg:px-8 ${selectedCandidateId ? "hidden md:block" : "block"}`}
        >
          ...
          <div className="space-y-3 md:space-y-4 lg:space-y-5 max-md:mt-5 mb-4 md:mb-6">
            {candidates.length > 0 ? (
              ...
              <p className="text-gray-600 text-center text-sm md:text-base">No candidates found for this job with the current filters.</p>
            )}
          </div>
          <Pagination
        ...
        <div className="hidden md:flex flex-1 max-w-[50%] md:ml-12 lg:ml-20">
          {selectedCandidate ? (
            ...
            <div className="flex items-center justify-center w-full h-full bg-white rounded-lg shadow-md">
              <p className="text-gray-500 text-sm md:text-base lg:text-lg">Select a candidate to view details.</p>
            </div>
          )}
        </div>
...
const mapApiCandidateToCardProps = (applicant: any): CandidateCardProps => {
  ...
  return {
    ...
    location: `${locationData.lga || ""}, ${locationData.state || ""}`.trim() || "Location not specified",
