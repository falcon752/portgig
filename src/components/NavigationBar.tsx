...
import { getRecruiterDashboard, type RecruiterDashboardData } from "../lib/requests/recruiterApi"
...
const getRecruiterNavigationItems = (dashboardData: RecruiterDashboardData | null) => {
  const jobPostedItems =
    dashboardData?.job_titles?.map((jobTitle) => ({
      label: jobTitle,
      link: `/recruiter-dashboard/jobs/${jobTitle.toLowerCase().replace(/\s+/g, "-")}`,
    })) || []
...
const NavigationBar = () => {
  ...
  const handleMessageClick = () => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(`messageLastViewed_${currentUserId}`, messageState.totalUnread.toString())
    }
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && currentUserId) {
      const persistedLastViewed = localStorage.getItem(`messageLastViewed_${currentUserId}`)
      const lastViewedCount = persistedLastViewed ? Number.parseInt(persistedLastViewed) : 0
  ...
  return (
    <nav className="sticky w-full top-0 z-50 h-14 text-primary bg-white">
      <div className="bodyMargin h-full flex items-center justify-between">
        ...
        <div className="lg:hidden flex items-center justify-between w-full">
          <div className="flex items-center gap-2">
            <button onClick={toggleMobileMenu} className="shrink-0">
              <Image src="/assets/nav-menu.svg" alt="Menu Icon" width={32} height={32} />
            </button>
            <Link href={homeLink}>
              <Image
                src="/assets/PortgigLogo.png"
        ...
        {/* Desktop Nav Links */}
        <div className="max-lg:hidden w-[600px] font-semibold text-primary text-[18px] font-urbanist">
          <ul className="flex justify-between">
            {navigationItems.map((item, index) => {
              ...
              const href = item.label === "Home" ? homeLink : item.label === "Messages" ? "/chats" : item.link
        ...
        <div className="max-lg:hidden flex items-center">
          {!profile && !recruiterProfile ? (
            ...
            <div className="relative">
              ...
              >
                <span>Hey, {profile?.bio_data?.user_name || recruiterProfile?.bio_data.full_name}</span>
              </button>
              {profileModal && (
                <div className="absolute top-14 left-0 bg-white rounded-lg px-2 py-1 w-[150px] z-50 flex flex-col justify-start items-start gap-3 shadow-lg border">
                  <button
                    type="button"
                    onClick={() => {
                      const dashboardRoute = userType === "creator" ? "/creative-dashboard" : "/recruiter-dashboard"
          ...
          <>
            ...
            <motion.div
              className={`fixed top-0 left-0 w-[280px] h-full shadow-lg z-50 flex flex-col px-6 py-8 ${
                isRecruiter ? "bg-[#0A1754] text-white" : "bg-white text-primary"
              }`}
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
            ...
            >
              ...
              <div className="flex justify-between items-center mb-8">
                <Link href={homeLink}>
                  <Image
                    src={isRecruiter ? "/assets/white-logo.png" : "/assets/PortgigLogo.png"}
                    alt="Portgig Logo"
                    quality={100}
                    width={100}
                    height={70}
                ...
                <button onClick={toggleMobileMenu}>
                  <AiOutlineClose className={`w-6 h-6 ${isRecruiter ? "text-white" : "text-black"}`} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto font-urbanist flex flex-col">
                {isRecruiter ? (
                  <>
                    {pathname === "/recruiter-dashboard/recruiter-edit-profile" ||
                    pathname === "/recruiter-dashboard/recruiter-change-password" ? (
                      <>
                        {/* Account Settings */}
                        <div className="flex-1">
                          <div className="mb-6 border-b border-white">
                            <h3 className="text-xl font-semibold text-white text-center">Account Settings</h3>
                          </div>
                        ...
                        <div className="pb-4 flex flex-col items-center cursor-pointer gap-3 mt-8">
                          ...
                          <p className="font-inter text-xs text-white text-center">Report an issue/Contact support</p>
                        </div>
                      </>
                    ) : (
                      // Dynamic Recruiter Navigation
                      <div className="space-y-8">
                        {getRecruiterNavigationItems(dashboardData).map((section, sectionIndex) => (
                          <div key={sectionIndex}>
                            {/* Section Header */}
                            <div className="mb-6 border-b border-white">
                              <h3 className="text-xl font-semibold text-white text-center">{section.category}</h3>
                            </div>
                            {/* Section Items */}
                            {section.items.length > 0 ? (
                                    ...
                                    {section.category === "Messages" ? (
                                      <Link
                                        ...
                                        className={`block text-base transition-colors duration-200 ${
                                          pathname === item.link ? "text-white font-semibold" : "text-white/90"
                                        } hover:text-white`}
                                      >
                                        {item.label}
                                        {unreadMessageCount > 0 && (
                              ...
                              <p className="text-white/70 text-sm ml-4">
                                {section.category === "Job Posted" && "No jobs posted yet"}
                                {section.category === "Applicants" && "No applicants yet"}
                                {section.category === "Messages" && "No messages"}
                        ...
                        <li key={index} className="relative">
                          {item.label === "Messages" ? (
                            <Link
                              href="/chats"
                              onClick={handleMessageClick}
                              className={`block ${pathname === "/chats" ? "text-primary font-bold" : "text-gray-700"}`}
                            >
                              {item.label}
                              {unreadMessageCount > 0 && (
                                <span className="ml-2 bg-red-500 text-white rounded-full w-5 h-5 inline-flex items-center justify-center text-xs">
                            ...
                            <Link
                              href={href}
                              className={`block ${pathname === href ? "text-primary font-bold" : "text-gray-700"}`}
                              onClick={() => setIsMobileMenuOpen(false)}
                            >
                              {item.label}
                            </Link>
                ...
                {!profile && !recruiterProfile ? (
                  ...
                  <button
                    ...
                    className={`text-left w-full py-3 px-4 rounded-full text-sm font-medium border ${
                      isRecruiter ? "text-red-300 border-red-500 hover:bg-red-300/10" : ""
                    }`}
                    disabled={isLoggingOut}
                  >
                    {isLoggingOut ? "Logging out..." : "Logout"}
