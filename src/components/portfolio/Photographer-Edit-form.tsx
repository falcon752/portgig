...
export default function PhotographerForm() {
  ...
  const { saveDraft, saveServerSnapshot, loadDraft, clearDraft, hasMeaningfulDraft } =
    useDraft<PhotographerFormData>("portgig_draft_photographer");
        ...
        if (portfolio && portfolio.template_type === "PHOTOGRAPHER") {
          ...
          if (isPhotographerTemplateSpecific(templateSpecific)) {
            setFormData({
              ...
              myServices: [
                ...
                ...Array(
                  Math.max(
                    0,
                    3 -
                      (templateSpecific.photographer?.my_services?.length || 0)
                  )
                ).fill({
                  image: { previewUrl: null, file: null },
                  name: "",
              ...
              latestWork: [
                ...
                ...Array(
                  Math.max(
                    0,
                    4 -
                      (templateSpecific.photographer?.latest_work?.length || 0)
                  )
                ).fill({
                  image: { previewUrl: null, file: null },
                  title: "",
              ...
              jobsOpenTo: (
                templateSpecific.photographer?.jobs_open_to || []
              ).join(", "),
              ...
              moreWork: [
                ...
                ...Array(
                  Math.max(
                    0,
                    4 - (templateSpecific.photographer?.more_work?.length || 0)
                  )
                ).fill({ name: "", link: "" }),
              ].slice(0, 4),
            });
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const displayNameSlug = formData.displayName
        ...
        : (username || "portfolio");
      portfolioUrl = `${window.location.origin}/photographer-portfolio/${encodeURIComponent(displayNameSlug)}/${encodeURIComponent(userId)}`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const jobsOpenToArray = formData.jobsOpenTo
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      ...
      const _photoNavSlug = formData.displayName ? formData.displayName.trim().toLowerCase().replace(/\s+/g, "-") : "portfolio";
      router.push(`/photographer-portfolio/${encodeURIComponent(_photoNavSlug)}/${encodeURIComponent(userId || "")}`);
      clearDraft();
    } catch (error: unknown) {
      if (error instanceof Error) {
        ...
        template_specific: {
          photographer: {
            ...
            latest_work: formData.latestWork
              .filter(
                (item) => item.image.previewUrl || item.title || item.link
              )
              .map((item) => ({
                image: item.image.previewUrl || "",
                title: item.title,
            ...
            more_work: formData.moreWork.filter(
              (item) => item.name || item.link
            ),
          },
        },
        ...
        <div>
          ...
          <div className="space-y-6">
            {formData.myServices.map((service, index) => (
              ...
              >
                <div className="aspect-video bg-gray-100 rounded-lg flex flex-col items-center justify-center border-2 border-dashed border-gray-300 hover:border-[#0A1754] transition cursor-pointer relative overflow-hidden">
                  {service.image.previewUrl ? (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          handleImageFileChange(e, "myServices", index)
                        }
                        className="hidden"
                        disabled={isAnyLoading}
                      />
                  ...
                  {item.image.previewUrl ? (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          handleImageFileChange(e, "latestWork", index)
                        }
                        className="hidden"
                        disabled={isAnyLoading}
                      />
